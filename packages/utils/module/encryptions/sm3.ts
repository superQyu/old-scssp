...
SM3.prototype.write = function (msg) {
  const m = typeof msg === 'string' ? this.strToBytes(msg) : msg;
  this.size += m.length;
...
SM3.prototype._compress = function (m) {
  ...
  const w = this._expand(m);
  const r = this.reg.slice(0);
  for (let j = 0; j < 64; j++) {
    let ss1 = this._rotl(r[0], 12) + r[4] + this._rotl(this._t(j), j);
    ss1 = (ss1 & 0xffffffff) >>> 0;
    ss1 = this._rotl(ss1, 7);

    const ss2 = (ss1 ^ this._rotl(r[0], 12)) >>> 0;
    let tt1 = this._ff(j, r[0], r[1], r[2]);
    tt1 = tt1 + r[3] + ss2 + w[j + 68];
    tt1 = (tt1 & 0xffffffff) >>> 0;
    let tt2 = this._gg(j, r[4], r[5], r[6]);
    tt2 = tt2 + r[7] + ss1 + w[j];
